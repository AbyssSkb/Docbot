...
class EvaluationTest(unittest.TestCase):
    def test_decision_metrics_only_include_generated_answers(self):
        rows = [
            gold("generated", answerable=True, chunks=["a"]),
            gold("retrieval", answerable=True, chunks=["b"]),
        ]
        predictions = [
            prediction("generated", ["a"], [], False, include_answer=False),
            prediction("retrieval", ["b"], [], False, include_answer=False),
        ]
        overall = evaluator.evaluate(rows, predictions)["overall"]
        self.assertNotIn("answerable", overall)
        self.assertNotIn("refusal", overall)
        predictions[0] = prediction("generated", ["a"], ["a"], True)
        overall = evaluator.evaluate(rows, predictions)["overall"]
        self.assertEqual(overall["answerable"]["recall"], 1.0)
        self.assertEqual(overall["generation"]["answer_correct"]["eligible_cases"], 1)
    ...
    def test_gold_without_metadata_keeps_prediction_matching_when_reordered(self):
        ...
        for row in rows:
            for field in ("question_id", "answerable"):
                del row[field]
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(all("split" not in row for row in loaded))
            ...
            self.assertNotIn("by_category", result)
    ...
    def test_metrics_slices_all_gold_rows(self):
        gold_rows = [
            ...
            gold("q3", answerable=True, chunks=["c"]),
        ]
        predictions = [
            prediction(
                "q1",
        ...
        self.assertNotIn("by_category", result)
        self.assertNotIn("by_split", result)
    ...
    def test_removed_review_fields_are_rejected_and_unretrieved_citation_is_wrong(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for field in ("status", "reviewer", "category", "group_id", "split"):
    ...
    def test_evaluation_requires_predictions_for_the_entire_dataset(self):
        gold_rows = [
            gold("first", answerable=True, chunks=["a"]),
            gold("second", answerable=True, chunks=["b"]),
        ]
        predictions = [prediction("first", ["a"], ["a"], True)]

        with self.assertRaisesRegex(ValueError, "missing predictions: second"):
            evaluator.evaluate(gold_rows, predictions)

        predictions.append(prediction("second", ["b"], ["b"], True))
        result = evaluator.evaluate(gold_rows, predictions)
        self.assertEqual(result["overall"]["cases"], 2)
        self.assertEqual(result["overall"]["retrieval"]["recall@10"], 1.0)
