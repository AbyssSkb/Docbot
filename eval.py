...
GOLD_OPTIONAL = {"notes", "question_id", "answerable"}
PREDICTION_REQUIRED = {
    "question_id",
    "retrieved_chunk_ids",
    "cited_chunk_ids",
...
def _validate_gold(record, location):
    ...
    _nonempty_string(record["question"], "question", location)
...
def load_gold(path):
    return _load_jsonl(path, _validate_gold)
...
def validate_gold_mapping(gold_records, chunks):
...
def validate_index_mapping(gold_records, predictions, index_dir):
    from pipeline import load_index_bundle

    _, chunks = load_index_bundle(index_dir)
    validate_gold_mapping(gold_records, chunks)
...
def _metrics(gold_rows, prediction_by_id, k):
    ...
    return {
        ...
        **({
            "answerable": _classification(generated_rows, prediction_by_id, "answerable"),
            "refusal": _classification(generated_rows, prediction_by_id, "refusal"),
...
def evaluate(gold_records, predictions, k=10):
    if k < 1:
    ...
    missing = all_gold_ids - prediction_by_id.keys()
    if missing:
    ...
    return {
        ...
        "counts": {
            ...
            "gold_cases": len(gold_records),
            "predictions": len(predictions),
        },
        "overall": _metrics(gold_records, prediction_by_id, k),
    }
